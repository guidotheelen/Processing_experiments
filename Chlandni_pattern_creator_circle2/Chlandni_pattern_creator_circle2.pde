...
void drawPattern() {
  ...
  for (int i = 0; i < density; i++) {
    ...
    float sampleU = u + patternOffsetX;
    float sampleV = v + patternOffsetY;
...
void drawUI() {
  ...
  fill(180);
  textSize(12);
  text("Drag inside the circle to navigate the pattern", panelX, 48);
  ...
  text(
    ...
    "offset x = " + nf(patternOffsetX, 1, 2) + "\n" +
    "offset y = " + nf(patternOffsetY, 1, 2),
    panelX,
    height - 140
  );
}
...
void mouseDragged() {
  ...
  if (panningPattern) {
    ...
    patternOffsetX -= (dx / plateDiameter) * panScale;
    patternOffsetY -= (dy / plateDiameter) * panScale;
