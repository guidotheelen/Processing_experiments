...
void draw() {
  ...
  for (int y = 0; y < rh; y++) {
    ...
    for (int x = 0; x < rw; x++) {
      // 4-way symmetry: mirror coordinates into the top-left quadrant.
      float mirroredX = min(x, rw - 1 - x);
      float mirroredY = min(y, rh - 1 - y);

      float px = mirroredX * noiseScaleAtRender;
      float pySym = mirroredY * noiseScaleAtRender;

      float warpA = noise(px * 0.90 + t * 0.32, pySym * 0.85 - t * 0.24, t * 0.15);
      float warpB = noise(px * 1.15 - t * 0.20 + 41.2, pySym * 1.10 + t * 0.29 + 20.8, -t * 0.13);
      float fx = px + (warpA - 0.5) * 1.45;
      float fy = pySym + (warpB - 0.5) * 1.45;

      // Two-layer domain-warped noise keeps curves flowing without strobing.
      float nA = noise(fx + t * 0.72, fy + t * 0.50, t * 0.20);
      float nB = noise(fx * 1.70 + 31.7, fy * 1.55 + 88.3, -t * 0.15);

      float field = nA * 0.70 + nB * 0.30;
      float contour = abs(sin((field * 18.0 + t * 0.85) * PI));
      float lineMask = smoothStep(0.30, 0.02, contour);
      float haloMask = smoothStep(0.70, 0.12, contour);

      float swirl = sin((nA * 1.8 + nB * 1.4) * TWO_PI + t * 1.5);
      float bands = sin(field * TWO_PI * 3.9 + t * 1.6);
      float pulse = 0.5 + 0.5 * sin(t * 4.2 + nB * TWO_PI);
      ...
      lowResBuffer.pixels[rowStart + x] = color((hueValue % 360 + 360) % 360, saturation, brightness);
    }
  }
