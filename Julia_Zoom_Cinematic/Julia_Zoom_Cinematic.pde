/*
  Julia_Zoom_Cinematic.pde
  ------------------------
  Widescreen Julia-set zoom renderer for making a long video.
  ...
  After rendering, build a video with ffmpeg (example):
    ffmpeg -framerate 30 -i frame-%07d.png -c:v libx264 -pix_fmt yuv420p julia_zoom.mp4

  Notes:
  - Rendering deep zooms is CPU-heavy.
  - Increase/decrease TOTAL_FRAMES or size for faster/slower workflows.
*/

// ------------------------------------------------------------
// Video/export settings
// ------------------------------------------------------------
int RENDER_WIDTH = 1280;   // 16:9 wide screen
int RENDER_HEIGHT = 720;   // change to 1920x1080 for Full HD
int FPS = 30;
int TOTAL_FRAMES = 2400;   // 80 seconds at 30 fps
boolean SAVE_FRAMES = true;
String FRAME_PATTERN = "frame-#######.png";
...
double finalZoom = 2.0e7;

// ------------------------------------------------------------
// Quality controls
// ------------------------------------------------------------
int baseIterations = 180;
int maxIterations = 4200;
...
void setup() {
  ...
  surface.setTitle("Julia Zoom Cinematic Renderer");
}
...
void renderZoomFrame(int i) {
  ...
  */
  double easedZoomT = easeInOutCubic(t);
  double easedDriftT = easeInOutQuad(t);
...
int adaptiveIterationCount(double zoom) {
  ...
  int adaptive = baseIterations + (int) (zoomDepth * 22.0);
